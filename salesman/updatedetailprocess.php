	...
	if (isset($_POST["submit"]))
	{
		if( empty($_POST["name"]) || empty($_POST["id"])|| empty($_POST["phone_number"])|| empty($_POST["state"])|| empty($_POST["city"])|| empty($_POST["pincode"])|| empty($_POST["salary"])|| empty($_POST["salesman_number"]) )
			...
			else
			{
				...
				if($result)
				{
					echo '<script type="text/javascript">alert("Entry Updated.");window.location.href="detail.php";</script>';
				}
				else{
					echo '<script type="text/javascript">alert("Can not update. May be samething missing please check again.");window.location.href="updatedetailphp.php";</script>';
