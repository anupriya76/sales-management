...
if(!$row)
{
	echo '<script type="text/javascript">alert("Id Mentioned is nor registered. Please Add This Id To your Team First.");window.location.href="updatesalesman.php";</script>';
}
?>					
<!DOCTYPE html>
<html lang="en" >
  ...
  <body>
<form class="box" action="updateprocess.php" method="post"autocomplete="off" >
 ...
 <!-- <input type="email" name="" oninvalid="setCustomValidity('not correct mail')"  placeholder="Email">-->
  <input pattern="\d{10}" id="no"  type="text" name="phone_number" oninvalid="setCustomValidity(''); checkValidity(); setCustomValidity(validity.valid?'':'Please enter 10 numbers only. ');"   value="<?php echo $row["PHONE_NUMBER"] ?>">
 ...
 <!-- <input placeholder="Birthday" class="textbox-n" type="text" onfocus="(this.type='date')" id="date"> -->
  ...
  <h1>Other Details</h1>
  <input pattern="[a-zA-Z0-9]+" type="text" id="px" name="id"   value="<?php echo $row["ID"] ?>">
<!--   <input placeholder="salesman_number"  type="text"  pattern="\d{4}" maxlength="4" oninvalid="setCustomValidity('not a valid year ')" id="ox"> -->
 ...
 <input  type="text"  value="<?php echo $row["SALARY"] ?>" name="salary" oninvalid="setCustomValidity('not a valid')" >
